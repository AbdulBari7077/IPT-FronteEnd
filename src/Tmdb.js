const API_KEY = '7271614f7d1457158f57f207fa5959e4';
...
export default {
    ...
    getTrailerVideo: async (movieId, type) => {
        ...
        if(movieId){
            switch(type){
                case 'movie':
                    trailer = await basicFetch(`/movie/${movieId}/videos?language=en&api_key=${API_KEY}`);
                break;
                case 'tv':
                    trailer = await basicFetch(`/tv/${movieId}/videos?language=en&api_key=${API_KEY}`);
                break;
                default: 
                    trailer = null;
                break;
